...
3) В левом меню навигации находим вкладку "Section", кликаем ПКМ и создаём новую секцию
...
8) В этой же вкладке (Section Hdrs) ищем секцию .text (секция с исполняемым кодом файла .exe) и запоминаем Raw Addr и Virtual Addr
  - [.text.]Raw Addr: 0x600
...
9) Создаём файл, содержащий ассэмблерный код секции .text и открываем
    ```
    File -> From all loaded -> Export Disassembly to -> сохраняем файл в папку с проектом
    Открываем файл sub.exe[.text].txt
    ```
10) В данном файле ищем инструкцию "call", связанную с нашей функцией, которую хотим заменить
...
15) Ищем в окне дизассемблера 2 команды, которые ранее мы уже находили в текстовом файле (команда mov для передачи адресса функции и команда call для перехода по этому адрессу), выделяем эти 2 команды и смотрим на их представление в окне с байт-кодом
...
17) Данные 2 команды можно заменить на 1 - вызов call нашей кастомной функции по адрессу Custom Addr. Байты, которые будут служить заменой выглядят следующим образом:
  ...
  18) В левом меню навигации находим название открытого файла (sub.exe), нажимаем ПКМ и сохраняем наши изменния в новый .exe файл
