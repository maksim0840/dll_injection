...
5) Заполняем информацию о секции (количество байт в нашей функции = 41 = 0x29):
    ...
    - Raw size (количество байт в секции, округлённое по кратности File_Alignment в большую сторону): 0x200
...
12) В данном файле ищем инструкцию "call", связанную с функцией, которую хотим заменить
